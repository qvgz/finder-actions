...
本文档是 Finder Actions 的技术事实来源，面向维护者和参与开发的 AI。开始修改代码前应先阅读本文；架构、约束、行为或工作流发生变化时，必须同步更新本文。面向普通使用者的安装和使用说明只写入英文 `README.md` 与中文 `README-zh.md`，两份用户文档必须保持内容同步。
...
刷新目录时，以当前扫描结果为准更新已启用操作的名称和说明，保留原有启用顺序，移除已经不存在或不安全的操作。新脚本不会自动加入已启用列表。用户通过列表行右侧的拖动把手调整数组顺序；放下时保存一次共享偏好，Finder 下次构建菜单时直接使用该顺序。
...
`interfaceLanguage` 有值时始终优先使用用户选择。尚未保存用户选择时，读取 `Locale.preferredLanguages.first`：所有 `zh` 中文变体使用简体中文界面，其他系统语言统一使用英文。不要在首次启动时显示额外语言选择页；用户可通过 `Language / 语言` 覆盖系统匹配结果。
...
6. 清除 `interfaceLanguage` 后分别验证中文系统自动使用中文、其他系统语言自动使用英文，并验证用户手动选择始终优先
...
用户可见的安装、操作或排障步骤发生变化时，还必须同步更新英文 `README.md` 和中文 `README-zh.md`。
