...
导入时应用会复制脚本并自动赋予执行权限。自定义脚本以当前用户权限运行，因此只应添加可信脚本。“添加右键功能 → 打开脚本文件夹”可查看和管理文件；新增或编辑脚本后，点击同一菜单中的“刷新功能列表”即可重新扫描。

应用会自动保护功能文件夹，并且只运行由当前用户拥有、不是符号链接且没有开放给其他用户写入的普通脚本。格式或安全设置不符合要求的文件会被忽略，刷新时会用易懂的提示告知用户，无需手动配置安全选项。
